    ...
    for file_item in file_list:
        ...
        try:
            ...
            if ext == ".pdf":
                ...
                text = f"[Image file: {orig_name} — image is attached and available for vision-capable LLMs]"
            else:
...
# --- Direct Chat Mode (no browser) ---


async def _run_direct_chat(
        webui_manager: WebuiManager, components: Dict[gr.components.Component, Any], task: str
) -> AsyncGenerator[Dict[gr.components.Component, Any], None]:
    """
    Direct LLM call — no Chromium browser is launched.
    Used when the user asks a question that doesn't require web browsing
    (e.g. text generation, math, summaries, simple Q&A).
    
    Args:
        webui_manager: The WebUI manager instance
        components: Dict of UI components and their values
        task: The user task (with file context already injected if files were uploaded)
    """
    ...
    # Show "thinking" state
    yield {
        user_input_comp: gr.update(value="", interactive=False, placeholder="Getting response…"),
        run_button_comp: gr.update(value="⏳ Thinking…", interactive=False),
    ...
    ollama_num_ctx = get_setting("ollama_num_ctx", 8192)
    ...
    try:
        ...
        logger.info(f"[Direct Chat] {llm_provider_name}/{llm_model_name} — task: {task[:80]}")
        response = await llm.ainvoke([HumanMessage(content=task)])
        answer = response.content if hasattr(response, "content") else str(response)
        webui_manager.bu_chat_history.append({"role": "assistant", "content": answer})
        ...
        gr.Error(f"Direct chat failed: {e}")
...
async def run_agent_task(
    ...
    task_raw = components.get(user_input_comp, "").strip()
    if not task_raw:
        gr.Warning("Please enter a task.")
        yield {run_button_comp: gr.update(interactive=True)}
        return

    # --- Append any uploaded file content to the task ---
    uploaded_files_comp = webui_manager.id_to_component.get("browser_use_agent.uploaded_files")
    uploaded_files_val = components.get(uploaded_files_comp) if uploaded_files_comp else None
    file_context = _parse_uploaded_files(uploaded_files_val)
    task = task_raw + (file_context if file_context else "")

    # --- Smart Mode Routing ---
    agent_mode_comp = webui_manager.id_to_component.get("browser_use_agent.agent_mode")
    agent_mode_val = (
        components.get(agent_mode_comp, "🤖 Browser Automation")
        if agent_mode_comp else "🤖 Browser Automation"
    )
    # If Direct Chat is selected, skip browser entirely
    if "Direct Chat" in agent_mode_val:
        webui_manager.bu_chat_history.append({"role": "user", "content": task})
        async for update in _run_direct_chat(webui_manager, components, task):
            yield update
        return
    ...
    if mcp_server_config_str and mcp_server_config_str.strip():
        try:
            mcp_server_config = json.loads(mcp_server_config_str)
        except Exception as e:
            logger.error(f"Failed to parse MCP server config: {e}")
            gr.Error(f"Invalid MCP Server JSON format. Note: Windows paths need double backslashes (C:\\\\Users). Error: {e}")
            yield {run_button_comp: gr.update(interactive=True)}
            return

    # Planner LLM Settings (Optional — leave empty for faster responses)
    ...
    if planner_llm_provider_name:
        ...
        if planner_llm_provider_name == "ollama":
            logger.info(
                "Planner LLM set to Ollama — this will double inference time per step. "
                "Leave Planner LLM Provider empty for faster single-model operation."
            )
        ...
        try:
            ...
            logger.warning(f"Failed to initialize Planner LLM, continuing without planner: {e}")
            gr.Warning(f"Planner LLM initialization failed: {e}. Running without planner.")
            planner_llm = None
    ...
    stealth_mode = get_browser_setting("stealth_mode", True)   # default ON
    ...
    if not webui_manager.bu_controller:
        ...
        # --- Show MCP connection status in chat ---
        if mcp_server_config:
            ctrl = webui_manager.bu_controller
            status_parts = []
            if ctrl.mcp_connected_servers:
                status_parts.append(
                    f"**✅ MCP Connected** — {len(ctrl.mcp_connected_servers)} server(s): "
                    f"{', '.join(ctrl.mcp_connected_servers)} "
                    f"({ctrl.mcp_total_tools} total tools)"
                )
            if ctrl.mcp_failed_servers:
                for fail in ctrl.mcp_failed_servers:
                    status_parts.append(
                        f"**⚠️ MCP Server '{fail['name']}' failed:** {fail['error']}"
                    )
            if not ctrl.mcp_connected_servers and not ctrl.mcp_failed_servers:
                status_parts.append(
                    "**ℹ️ MCP:** No servers configured (placeholder values skipped)."
                )
            if status_parts:
                webui_manager.bu_chat_history.append({
                    "role": "assistant",
    ...
    try:
        ...
        if not webui_manager.bu_browser:
            ...
            # Pass stealth preference via a sentinel in extra_browser_args.
            # custom_browser/_context reads this to decide whether to apply patches.
            if not stealth_mode:
                extra_args += ["--no-stealth"]   # sentinel: Chrome ignores unknown args
        ...
        # Track last N action fingerprints for stuck-loop detection
        _recent_action_hashes: list[str] = []
        _STUCK_REPEAT_LIMIT = 3  # Stop if same action repeated this many times in a row
        ...
        async def step_callback_wrapper(
            ...
            # Serialize the current step's actions to a comparable fingerprint
            try:
                action_fingerprint = str([a.model_dump(exclude_none=True) for a in output.action])
            except Exception:
                action_fingerprint = ""
            ...
            if action_fingerprint:
                ...
                if len(_recent_action_hashes) > _STUCK_REPEAT_LIMIT:
                    _recent_action_hashes.pop(0)

                # All N are identical → agent is stuck
                if (
                    len(_recent_action_hashes) == _STUCK_REPEAT_LIMIT
                    and len(set(_recent_action_hashes)) == 1
                ):
                    logger.warning(
                        f"[StuckDetector] Same action repeated {_STUCK_REPEAT_LIMIT} times at step {step_num}: "
                        f"{action_fingerprint[:120]}. Stopping agent."
                    )
        ...
        # --- Build MCP-aware system prompt extension ---
        # When MCP tools are available, guide the agent to use them for
        # local file/folder operations instead of trying to open VS Code.
        mcp_system_ext = ""
        if (webui_manager.bu_controller
                and hasattr(webui_manager.bu_controller, 'mcp_connected_servers')
                and webui_manager.bu_controller.mcp_connected_servers):
            mcp_tool_names = [
                name for name in webui_manager.bu_controller.registry.registry.actions.keys()
                if name.startswith("mcp.")
            ]
            mcp_system_ext = (
                "\n\n## MCP Tools Available\n"
                "You have access to MCP (Model Context Protocol) tools for local operations. "
                "CRITICAL RULES when MCP tools are available:\n"
                "1. For ANY file/folder creation, editing, reading, or writing tasks — "
                "USE the MCP filesystem tools directly (e.g. mcp.filesystem.write_file, "
                "mcp.filesystem.create_directory, mcp.filesystem.read_file). "
                "DO NOT open VS Code, Notepad, or any text editor in the browser.\n"
                "2. For running local servers (e.g. localhost) — use MCP tools to create "
                "the files first, then guide the user to run the server command.\n"
                "3. DO NOT search Google or Bing for 'VS Code download' or 'how to open VS Code'. "
                "The filesystem tools give you direct access.\n"
                "4. When asked to create HTML/CSS/JS files, use mcp.filesystem.write_file "
                "to write the content directly to the specified path.\n"
                "5. After creating files, tell the user the exact path and how to open/run them.\n\n"
                f"Available MCP tools: {', '.join(mcp_tool_names[:30])}\n"
            )

        # Combine user's extend_system_prompt with MCP extension
        final_extend_prompt = (extend_system_prompt or "") + mcp_system_ext
        ...
        if not webui_manager.bu_agent:
            ...
            webui_manager.bu_agent = BrowserUseAgent(
                ...
                extend_system_message=final_extend_prompt if final_extend_prompt.strip() else None,
                ...
                source="webui",
                enable_memory=False,  # Disable mem0: text-embedding-004 unavailable in Gemini v1beta
            )
            ...
            final_update.update(
                {
                    ...
                    chatbot_comp: gr.update(value=webui_manager.bu_chat_history),
                    # Clear browser view so it does not freeze on last screenshot
                    browser_view_comp: gr.update(
                        value="<div style='text-align:center;padding:40px;opacity:0.5;'>✅ Task finished — start a new task above</div>",
                        visible=True,
                    ),
                }
            )
...
async def handle_submit(
    ...
    # Check if a task is currently running (using _current_task)
        ...
        # Must yield updates for ALL output components to avoid Gradio validation error
        chatbot_comp = webui_manager.get_component_by_id("browser_use_agent.chatbot")
        run_button_comp = webui_manager.get_component_by_id("browser_use_agent.run_button")
        stop_button_comp = webui_manager.get_component_by_id("browser_use_agent.stop_button")
        pause_resume_button_comp = webui_manager.get_component_by_id("browser_use_agent.pause_resume_button")
        ...
        yield {
            ...
            user_input_comp: gr.update(),
            run_button_comp: gr.update(),
            stop_button_comp: gr.update(),
            pause_resume_button_comp: gr.update(),
            clear_button_comp: gr.update(),
            history_file_comp: gr.update(),
            gif_comp: gr.update(),
            browser_view_comp: gr.update(),
        }
...
async def handle_clear(webui_manager: WebuiManager):
    ...
    return {
        ...
        webui_manager.get_component_by_id("browser_use_agent.uploaded_files"): gr.update(
            value=None
        ),
    }
...
def create_browser_use_agent_tab(webui_manager: WebuiManager):
    ...
    with gr.Column():
        agent_mode = gr.Radio(
            choices=["🤖 Browser Automation", "💬 Direct Chat (No Browser)"],
            value="🤖 Browser Automation",
            label="Agent Mode",
            info=(
                "🤖 Browser Automation — launches a real Chromium browser to click, navigate, "
                "fill forms, and extract info from any website.  "
                "💬 Direct Chat — plain LLM answer with no browser launched. Best for text "
                "generation, summaries, coding help, math, and simple Q&A."
            ),
            interactive=True,
        )
        chatbot = gr.Chatbot(
        ...
        uploaded_files = gr.File(
            label="📎 Attach Files (PDF, Word, TXT, CSV, Images)",
            file_count="multiple",
            file_types=[".pdf", ".txt", ".docx", ".doc", ".csv",
                        ".md", ".json", ".html", ".png", ".jpg", ".jpeg"],
            interactive=True,
            height=80,
        )
        with gr.Row():
