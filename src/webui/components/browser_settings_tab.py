...
def create_browser_settings_tab(webui_manager: WebuiManager):
    ...
    with gr.Group():
        with gr.Row():
            ...
            stealth_mode = gr.Checkbox(
                ...
                value=True,
                info=(
                    "Enable anti-bot evasion (recommended). "
                    "Patches navigator.webdriver, Chrome fingerprints, WebGL, permissions and ~15 other "
                    "signals that sites like Ticketmaster, IRCTC, and airline portals use to detect bots. "
                    "Disable only if it causes issues with a specific site."
                ),
                interactive=True,
            )
    ...
    stealth_mode.change(close_wrapper)
