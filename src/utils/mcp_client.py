...
async def setup_mcp_client_and_tools(mcp_server_config: Dict[str, Any]) -> Optional[MultiServerMCPClient]:
    """
    Initializes the MultiServerMCPClient with the provided server config.
    Compatible with langchain-mcp-adapters >= 0.1.0 (no-context-manager API).

    This function only configures the client — it does NOT open sessions or
    fetch tools eagerly (sessions are opened on demand per-call to get_tools()).

    Common JSON fields that are NOT part of the MCP spec (e.g. "comment") are
    automatically stripped so existing config files keep working.

    Returns:
        MultiServerMCPClient: The configured client, or None on failure.
    """
    ...
    # Fields allowed by langchain-mcp-adapters per transport type
    _STDIO_KEYS  = {"command", "args", "env", "transport"}
    _HTTP_KEYS   = {"url", "headers", "transport"}
    _ALLOWED     = _STDIO_KEYS | _HTTP_KEYS

    # Regex patterns that identify unconfigured placeholder values
    _PLACEHOLDER_RE = re.compile(
        r"YOUR_[A-Z_]+_HERE|your-\S+-here|/path/to/|C:/path/to/"
        r"|xoxb-your-|YOUR_TEAM_ID|YOUR_API_KEY",
        re.IGNORECASE,
    )

    def _has_placeholders(cfg: dict) -> bool:
        """Return True if any env value or arg looks like an unconfigured placeholder."""
        for v in cfg.get("env", {}).values():
            if _PLACEHOLDER_RE.search(str(v)):
                return True
        for arg in cfg.get("args", []):
            if _PLACEHOLDER_RE.search(str(arg)):
                return True
        return False
    ...
    try:
        ...
        for server_name, server_cfg in mcp_server_config.items():
            # Strip unknown/comment fields the SDK cannot handle
            cfg = {k: v for k, v in server_cfg.items() if k in _ALLOWED}

            # Windows-specific fix for npx
            import sys
            if "command" in cfg and sys.platform == "win32":
                if cfg["command"] == "npx":
                    cfg["command"] = "npx.cmd"
                elif cfg["command"] == "npm":
                    cfg["command"] = "npm.cmd"

            # Strip empty env dicts — they add noise and can confuse sub-processes
            if "env" in cfg and not cfg["env"]:
                del cfg["env"]

            # Skip servers whose args/env still contain placeholder values
            if _has_placeholders(cfg):
                logger.warning(
                    f"MCP server '{server_name}' has unconfigured placeholder values "
                    f"(e.g. 'YOUR_*_HERE' or '/path/to/') — skipping. "
                    f"Replace them with real values in your JSON config."
                )
                continue

            # --- Backwards-compat: auto-add 'transport' for old-style configs ---
            if "transport" not in cfg:
                if "command" in cfg:
                    cfg["transport"] = "stdio"      # npx / uvx / python / etc.
                elif "url" in cfg and (
                    "sse" in cfg["url"] or cfg["url"].rstrip("/").endswith("/sse")
                ):
                    cfg["transport"] = "sse"
                elif "url" in cfg:
                    ...
                    logger.warning(
                        f"MCP server '{server_name}' has neither 'command' nor 'url' — skipping."
                    )
                    continue

            upgraded_config[server_name] = cfg
            logger.info(
                f"MCP server '{server_name}': transport={cfg['transport']}, "
                f"command={cfg.get('command','')}{cfg.get('url','')}"
            )

        if not upgraded_config:
            logger.error("No valid MCP server entries found after sanitisation.")
            return None

        # Instantiate client — sessions are opened lazily per get_tools() call
        client = MultiServerMCPClient(upgraded_config)
        logger.info(
            f"MultiServerMCPClient configured with {len(upgraded_config)} server(s): "
            f"{list(upgraded_config.keys())}"
        )
        return client
